...
* The `dateline-parsec-MOESI_hammer.tar.gz` file is submitted to the staging. And on untar also the folder name is `dateline-parsec-MOESI_hammer`.
* The `dateline-parsec-MOESI_hammer.tar.gz` has the gem5 already compiled. And it also has the scripts folder with contents same as automation-scripts (can check file paths in .sh script for more clarity).
